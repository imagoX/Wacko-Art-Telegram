...
def validate_url(url):
    ...
    if not url.startswith(('http://', 'https://')):
        url = 'https://' + url
    parsed = urlparse(url)
    if 'getdailyart.com' not in parsed.netloc and 'www.getdailyart.com' not in parsed.netloc:
        return None
...
def extract_image_and_description(dailyart_url):
    ...
    try:
        ...
        response = requests.get(dailyart_url, headers=headers, timeout=REQUEST_TIMEOUT, allow_redirects=True)
        logger.info(f"Requested {dailyart_url}, redirected to {response.url}, status: {response.status_code}")
        ...
        img_tag = main_image_div.find('img', class_=lambda x: x != 'main-image__blurred')
        ...
        if not main_desc:
            ...
            title = main_desc.find('h1', class_='main-description__title').text.strip() if main_desc.find('h1', class_='main-description__title') else "Untitled"
            artist = main_desc.find('span', class_='main-description__author').text.strip() if main_desc.find('span', class_='main-description__author') else "Unknown Artist"
            year = main_desc.find('span', class_='main-description__author-years').text.strip() if main_desc.find('span', class_='main-description__author-years') else "Unknown Year"
            attr = main_desc.find('div', class_='main-description__attr').text.strip() if main_desc.find('div', class_='main-description__attr') else ""
            museum = attr.split('cm')[-1].strip() if 'cm' in attr else "Unknown Location"
            short_desc = f"{title} by {artist}, {year}, {museum}"
        
        desc_content = main_desc.find('div', class_='main-description__text-content') if main_desc else None
        full_desc = desc_content.get_text(separator='\n').strip() if desc_content else "No detailed description available."
...
def download_image(url, temp_dir, index):
    ...
    try:
        ...
        if content_length > MAX_FILE_SIZE:
            return None, f"Image {index} too large ({content_length / 1024 / 1024:.1f}MB)"
...
async def send_image_selection(update: telegram.Update, context: ContextTypes.DEFAULT_TYPE, image_urls, descriptions, chat_id, message="Found images:"):
    """Send inline keyboard for user to select images."""
    keyboard = []
    for i, url in enumerate(image_urls, 1):
        keyboard.append([InlineKeyboardButton(f"Image {i}", callback_data=f"img_{i-1}")])
        ...
        for i in indices:
            ...
            if validate_url(url) and not url.endswith(('.jpg', '.png')):
                ...
                if new_urls:
                    ...
                    await context.bot.send_message(chat_id, f"Image {i + 1} extraction failed.")
                    continue
            ...
            if filename:
                ...
                await context.bot.send_message(chat_id, f"Image {i + 1} failed: {error}")
        ...
        for filename, index, url in downloaded_files:
            try:
                ...
                if file_size > MAX_FILE_SIZE:
                    await context.bot.send_message(chat_id, f"Image {index} too large ({file_size / 1024 / 1024:.1f}MB)")
                    continue
                
                short_desc = descriptions.get(url, ("Artwork from GetDailyArt", ""))[0]
                keyboard = [[InlineKeyboardButton("Explanation", callback_data=f"explain_{index-1}")]]
                reply_markup = InlineKeyboardMarkup(keyboard)
                ...
                await context.bot.send_message(chat_id, f"Error sending image {index}: {e}")
        
        await context.bot.send_message(chat_id, f"Sent {len(downloaded_files)} image(s)!")
...
async def handle_message(update: telegram.Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if normalized_url:
        ...
        try:
            image_urls, short_desc, full_desc = extract_image_and_description(normalized_url)
        ...
        if not image_urls:
            await update.message.reply_text("Couldn’t find any artwork images in that link.")
            return
        ...
        if len(image_urls) == 1:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if filename:
                    try:
                        keyboard = [[InlineKeyboardButton("Explanation", callback_data="explain_0")]]
                        reply_markup = InlineKeyboardMarkup(keyboard)
                        with open(filename, 'rb') as photo:
                            await context.bot.send_photo(
                                chat_id=chat_id,
                        ...
                        user_data[chat_id] = {'urls': image_urls, 'descriptions': {image_urls[0]: (short_desc, full_desc)}}
                    except telegram.error.TelegramError as e:
            ...
            await send_image_selection(update, context, image_urls, {image_urls[0]: (short_desc, full_desc)}, chat_id)
    else:
